...
public class ProductAdapter extends BaseAdapter {
	...
	public ProductAdapter(DActivity2 context, List<Product> list,
			StoreAdapter adapter, int storePosition) {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			holder.ll_quantity = (LinearLayout) convertView
					.findViewById(R.id.ll_quantity);
			...
			holder.ll_edit_quantity = (LinearLayout) convertView
					.findViewById(R.id.ll_edit_quantity);
			holder.btn_decrease = (Button) convertView
					.findViewById(R.id.btn_decrease);
			holder.btn_increase = (Button) convertView
					.findViewById(R.id.btn_increase);
			holder.et_quantity = (EditText) convertView
		...
		holder.btn_done.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				product.setCount(Integer.parseInt(et_quantity.getText()
						.toString()));
		...
		holder.et_quantity.setText(product.getCount() + "");
		holder.tv_quantity.setText(product.getCount() + "");
		...
		holder.btn_decrease.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				if (!(TextUtils.isEmpty(et_quantity.getText().toString()) || "1"
						.equals(et_quantity.getText().toString()))) {
					et_quantity.setText(Integer.parseInt(et_quantity.getText()
							.toString()) - 1 + "");
				}
			}
		});
