...
public class OnlyListViewSwipeGesture implements OnTouchListener {
	Activity activity;
	...
	String RangeOneColor = "#FF4444"; // "#FFD060"
	...
	public OnlyListViewSwipeGesture(ListView listView, TouchCallbacks Callbacks,
			Activity context) {
			...
			for (int i = 0; i < childCount; i++) {
				...
				if (rect.contains(x, y)) {
					...
					mDownView = (ViewGroup) child
							.findViewById(R.id.list_display_view_container);
	...
	class touchClass implements OnTouchListener {
		...
		public boolean onTouch(View v, MotionEvent event) {
			...
			// opened_position = mListView.getPositionForView((View)
			// v.getParent());
			switch (event.getActionMasked()) {
			case MotionEvent.ACTION_DOWN: {
				if (moptionsDisplay) {
