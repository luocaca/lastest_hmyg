...
public class WheelView extends View {
	...
	public WheelView(Context context, AttributeSet attrs, int defStyle) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.wheelViewStyle);
		TEXT_SIZE = a.getDimensionPixelSize(
				R.styleable.wheelViewStyle_textSize, 16);
		a.recycle();
	...
	private void initData(Context context, AttributeSet attrs) {
		...
		dip1px = AndroidUtil.dip2px(context, 1);
	}
	...
	private void drawCenterRect(Canvas canvas) {
		...
		canvas.drawRect(0, center - offset, getWidth(), center - offset
				+ dip1px, centerLinePaint);
		canvas.drawRect(0, center + offset - dip1px, getWidth(), center
				+ offset, centerLinePaint);
	}
	...
	private void doScroll(int delta) {
		if (!isCyclic) {
			if (getCurrentItem() == (adapter.getItemsCount() - 1) && delta < 0) {
				setCurrentItem(getCurrentItem(), false);
				return;
			} else if (getCurrentItem() == 0 && delta > 0) {
		...
		if (isCyclic && adapter.getItemsCount() > 0) {
			...
			if (!isCyclic) {
				if (pos == 0 && scrollingOffset > 0) {
					scrollingOffset = Math
							.min(scrollingOffset, getItemHeight());
				} else if ((pos == adapter.getItemsCount() - 1)
						&& scrollingOffset < 0) {
					scrollingOffset = Math.max(scrollingOffset,
							-getItemHeight());
				}
			}
		} else {
