	...
	private ArcConfiguration readFromAttributes(AttributeSet attributeSet) {
		...
		for (int i = 0; i < array.length(); i++) {
			...
			if (type == R.styleable.SimpleArcLoader_arc_style) {
				String value = array
						.getString(R.styleable.SimpleArcLoader_arc_style);

				configuration.setLoaderStyle(STYLE.values()[Integer
						.parseInt(value)]);
			}

			if (type == R.styleable.SimpleArcLoader_arc_colors) {
				int colors_resourse_id = array.getResourceId(
						R.styleable.SimpleArcLoader_arc_colors, 0);

				if (colors_resourse_id != 0)
					configuration.setColors(getContext().getResources()
							.getIntArray(colors_resourse_id));
			}

			if (type == R.styleable.SimpleArcLoader_arc_speed) {
				String value = array
						.getString(R.styleable.SimpleArcLoader_arc_speed);

				if (value != null)
					configuration.setAnimationSpeedWithIndex(Integer
							.parseInt(value));
			}

			if (type == R.styleable.SimpleArcLoader_arc_margin) {
				Float value = array.getDimension(
						R.styleable.SimpleArcLoader_arc_margin, MARGIN_MEDIUM);
			...
			if (type == R.styleable.SimpleArcLoader_arc_thickness) {
				Float value = array.getDimension(
						R.styleable.SimpleArcLoader_arc_thickness,
	...
	private class ArcDrawable extends Drawable implements Animatable {
		final Runnable updater = new Runnable() {
			...
			public void run() {
				...
				scheduleSelf(updater,
						FRAME_DURATION + SystemClock.uptimeMillis());
				invalidateSelf();
			}
		};
		...
		public void draw(Canvas canvas) {
			...
			RectF arc1_bound = new RectF(arc1_bound_start + arc_padding,
					arc1_bound_start + arc_padding,
					((w - (mStrokeWidth * 2)) - mArcMargin) - arc_padding,
					((h - (mStrokeWidth * 2)) - mArcMargin) - arc_padding);
			RectF arc2_bound = new RectF(mStrokeWidth + arc_padding,
					mStrokeWidth + arc_padding, (w - mStrokeWidth)
							- arc_padding, (h - mStrokeWidth) - arc_padding);
			...
			for (int i = 0; i < (colors_length > 4 ? 4 : colors_length); i++) {
				...
				canvas.drawArc(arc1_bound, startangle + mArcAnglePosition, 90,
						false, mPaint);
				canvas.drawArc(arc2_bound, startangle - mArcAnglePosition, 90,
						false, mPaint);
			}
		}
