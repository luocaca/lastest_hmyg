...
public class ElasticTouchListener implements OnTouchListener {
				...
				if (isNeedMove()) {
					...
					if (children != null) {
						...
						for (int i = 0; i < children.length; i++) {
							view = children[i];
							view.layout(view.getLeft(), view.getTop() - deltaY
									/ 2, view.getRight(), view.getBottom()
									- deltaY / 2);
						}
					} else {
						// 移动布局
						inner.layout(inner.getLeft(), inner.getTop() - deltaY
								/ 2, inner.getRight(), inner.getBottom()
								- deltaY / 2);
					}
				}
				inner.invalidate();
				...
				if (view.getVisibility() == View.VISIBLE) {
					...
					trans.setAnimationListener(new AnimationListener() {
						...
						public void onAnimationEnd(Animation animation) {
							view.clearAnimation();
							// 设置回到正常的布局位置
							view.layout(view.getLeft(), tops[index],
									view.getRight(), bottoms[index]);
							normal.setEmpty();
							animationFinish = true;
						}
					});
	...
	public boolean isNeedMove() {
		...
		// && (children[children.length - 1].getBottom() <=
		// inner.getPaddingTop()/*inner.getTop()*/
