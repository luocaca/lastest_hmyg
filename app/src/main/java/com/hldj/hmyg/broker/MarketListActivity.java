...
import com.hldj.hmyg.broker.adapter.MarketListAdapter;
...
@SuppressLint("NewApi")
public class MarketListActivity extends BaseSecondActivity implements
		IXListViewListener,
			...
			if (flag) {
				...
				case R.id.id_tv_edit_all:
					Intent toMyMarketListActivity = new Intent(
							MarketListActivity.this, MyMarketListActivity.class);
...
//					}
			...
			Intent toSellectActivity = new Intent(MarketListActivity.this,
					SellectMarketPriceActivity.class);
			sellectPrice = new SellectPrice();
			toSellectActivity.putExtra("sellectPrice", sellectPrice);
			startActivityForResult(toSellectActivity, 1);
	...
	public void onRefresh() {
		...
		datas.clear();
		if (listAdapter == null) {
			listAdapter = new MarketListAdapter(MarketListActivity.this, datas);
	...
	private void initData() {
		...
		finalHttp.post(GetServerUrl.getUrl() + "seedlingMarket/list", params,
								...
								if (JsonGetInfo.getJsonArray(jsonObject2,
									...
									for (int i = 0; i < jsonArray.length(); i++) {
										...
										hMap.setId(JsonGetInfo.getJsonString(
												jsonObject3, "id"));
										hMap.setFirstSeedlingTypeId(JsonGetInfo
												.getJsonString(jsonObject3,
														"firstSeedlingTypeId"));
										hMap.setSecondSeedlingTypeId(JsonGetInfo
												.getJsonString(jsonObject3,
														"secondSeedlingTypeId"));
										hMap.setFirstTypeName(JsonGetInfo
												.getJsonString(jsonObject3,
														"firstTypeName"));
										hMap.setQualityGrade(JsonGetInfo
												.getJsonString(jsonObject3,
														"qualityGrade"));
										hMap.setQualityGradeName(JsonGetInfo
												.getJsonString(jsonObject3,
														"qualityGradeName"));
										...
										if ("".equals(JsonGetInfo
											...
											hMap.setCityName(JsonGetInfo.getJsonString(
									...
									if (listAdapter == null) {
										listAdapter = new MarketListAdapter(
												MarketListActivity.this, datas);
										xListView.setAdapter(listAdapter);
										xListView
					...
					public void onSuccess(Object t) {
						...
						try {
							...
							if ("1".equals(code)) {
								...
								Toast.makeText(MarketListActivity.this, "删除失败",
	...
	private void showCitys() {
		...
		tv_sure.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				cityCode = GetCodeByName.initProvinceDatas(
						MarketListActivity.this, mCurrentProviceName,
						mCurrentCityName);
				onRefresh();
				if (!MarketListActivity.this.isFinishing() && dialog != null) {
					if (dialog.isShowing()) {
						dialog.cancel();
					} else {
						dialog.show();
		...
		if (!MarketListActivity.this.isFinishing() && dialog.isShowing()) {
			dialog.cancel();
		} else if (!MarketListActivity.this.isFinishing() && dialog != null
				&& !dialog.isShowing()) {
			dialog.show();
		}
	...
	private void setUpData() {
		...
		mViewProvince.setViewAdapter(new ArrayWheelAdapter<String>(
				MarketListActivity.this, mProvinceDatas));
		// 设置可见条目数量
