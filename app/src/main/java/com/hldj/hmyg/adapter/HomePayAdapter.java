...
public class HomePayAdapter extends BaseAdapter {
	...
	public HomePayAdapter(Context context, ArrayList<Type> data) {
		this.data = data;
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		inflate.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if ("weituo".equals(data.get(position).getId())) {
					Intent toWeituoActivity = new Intent(context,
							WeituoActivity.class);
					toWeituoActivity.putExtra("title", "委托购");
					toWeituoActivity.putExtra("url", Data.weituogou);
					context.startActivity(toWeituoActivity);
					...
					toBActivity.putExtra("supportTradeType", data.get(position)
							.getId());
					toBActivity.putExtra("supportTradeTypeName",
