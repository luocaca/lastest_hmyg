...
public class BuyerListAdapter extends BaseAdapter {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (!"".equals(data.get(position).get("acceptStatus").toString())) {
			...
			BuyerProductListAdapter adapternei = new BuyerProductListAdapter(
					context, cartList);
		...
		ll_item_broker.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				Intent toBuyerListDetailActivity = new Intent(context,
						BuyerListDetailActivity.class);
				toBuyerListDetailActivity.putExtra("id", data.get(position)
						.get("id").toString());
				context.startActivity(toBuyerListDetailActivity);
