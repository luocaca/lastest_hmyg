...
public class OrderDetailActivity extends NeedSwipeBackActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_saler_order_detail);
	...
	private void init() {
		...
		params.put("id", id);
		finalHttp.post(GetServerUrl.getUrl() + "admin/seller/order/detail",
			...
			if (flag) {
				...
				case R.id.layoutRoot:
					if (bean != null && !"".equals(bean.getId())) {
						Intent toFlowerDetailActivity = new Intent(OrderDetailActivity.this,
								FlowerDetailActivity.class);
						toFlowerDetailActivity.putExtra("id",
								bean.getId());
						toFlowerDetailActivity.putExtra("show_type", "seedling_list");
