...
public class FlowTagLayout extends ViewGroup {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		for (int i = 0, childCount = getChildCount(); i < childCount; i++) {
			...
			int realChildHeight = childHeight + mlp.topMargin
					+ mlp.bottomMargin;
...
//			}
	...
	private void reloadData() {
		...
		for (int i = 0; i < mAdapter.getCount(); i++) {
			...
			addView(childView, new MarginLayoutParams(
					LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT));

			if (mAdapter instanceof OnInitSelectedPosition) {
				boolean isSelected = ((OnInitSelectedPosition) mAdapter)
				...
				public void onClick(View v) {
					if (mTagCheckMode == FLOW_TAG_CHECKED_NONE) {
						if (mOnTagClickListener != null) {
							mOnTagClickListener.onItemClick(FlowTagLayout.this,
									childView, j);
						}
						...
						if (mCheckedTagArray.get(j)) {
							...
							if (mOnTagSelectListener != null) {
								mOnTagSelectListener.onItemSelect(
										FlowTagLayout.this,
						...
						if (mOnTagSelectListener != null) {
							mOnTagSelectListener.onItemSelect(
									FlowTagLayout.this, Arrays.asList(j));
						}
					} else if (mTagCheckMode == FLOW_TAG_CHECKED_MULTI) {
						if (mCheckedTagArray.get(j)) {
							mCheckedTagArray.put(j, false);
	...
	public void setCheckPos(int position) {
		...
		if (mCheckedTagArray.get(position)) {
			...
			childView.setSelected(true);
			if (mOnTagSelectListener != null) {
				mOnTagSelectListener.onItemSelect(FlowTagLayout.this,
						new ArrayList<Integer>());
			}
			return;
		}
		...
		// SparseBooleanArray mNewCheckedTagArray = new SparseBooleanArray();
		// for (int i = 0; i < mCheckedTagArray.size(); i++) {
		// if(i ==position){
		// mNewCheckedTagArray.put(position, true);
		// }else {
		// mNewCheckedTagArray.put(i, true);
		// }
		// }
		// mCheckedTagArray =mNewCheckedTagArray;
		// childView.setSelected(true);
		// ArrayList<Integer> arrayList = new ArrayList<Integer>();
		// arrayList.add(position);
		// if (mOnTagSelectListener != null) {
		// mOnTagSelectListener.onItemSelect(FlowTagLayout.this, arrayList);
		// }
	}
