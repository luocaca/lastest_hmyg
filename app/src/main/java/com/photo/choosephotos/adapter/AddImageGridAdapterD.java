...
public class AddImageGridAdapterD extends BaseAdapter implements
		DragGridBaseAdapter {
	// 定义Context
	...
	public AddImageGridAdapterD(Context context) {
		this.context = context;
	}

	public AddImageGridAdapterD(Context context, List<Bitmap> imageList,
			List<Item> photoList) {
		this.context = context;
	...
	public void reorderItems(int oldPosition, int newPosition) {
		Bitmap bitmap = imageList.get(oldPosition);
		Item item = photoList.get(oldPosition);
		...
		imageList.set(newPosition, bitmap);
		photoList.set(newPosition, item);
	}
